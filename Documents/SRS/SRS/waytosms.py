...
class sms:
    ...
    def __init__(self, username, password):
        ...
        self.jsid = self.s.cookies.get_dict()['JSESSIONID'][
                    4:]  # JSID is the main KEY as JSID are produced every time a session satrts
    ...
    def msgSentToday(self):
        ...
        self.soup = BeautifulSoup(self.q.text,
                                  'html.parser')  # we want the number of messages sent which is present in the
    ...
    def send(self, mobile_no, msg):
        ...
        if len(msg) > 139 or len(
                mobile_no) != 10 or not mobile_no.isdecimal():  # checks whether the given message is of length more than 139
